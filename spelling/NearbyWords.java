...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int i=0;i<s.length();i++)
		{
			...
			{
				String s2=s.substring(0,i)+Character.toString((char)j)+s.substring(i,s.length());
				if(wordsOnly==true)
				{ 
					if(dict.isWord(s2))
						currentList.add(s2);
		...
		for(int j=97;j<=122;j++)
		{
			String s2=s+Character.toString((char)j);
			if(wordsOnly==true)
			{ 
				if(dict.isWord(s2))
					currentList.add(s2);
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		visited.add(word);
		int i=0;
					while(queue.isEmpty()==false||i<numSuggestions)
					{
					 ...
					 queue.remove(0);
					 List<String> mut=distanceOne(curr,false);
					 for(String s:mut)
					 {
						 k++;
